...
$comsonglist=@file("$datadir/commend.php");
$singersum=count($singerlist);	
$songsum=count($comsonglist);

function get_commendsong($page){

global $datadir,$singerlist,$comsonglist,$singersum,$songsum,$perpage,$skinpath,$skin;
...
if($songsum!=0){
	...
	if ($page>$maxpageno) $page=$maxpageno;
		...
		for ($i=$pagemin; $i<=$pagemax; $i++){
		 $songdetail=explode("|",$comsonglist[$i]);
  ...
  echo "<li><div align=\"center\">
		<input type='button' value='全选' onclick='sal(commend)' class=\"sbt\">\n
&nbsp;&nbsp;<input type='button' value='反选' onclick='opal(commend)' class=\"sbt\">\n
&nbsp;&nbsp;<input type='button' value='清空' onclick='clal(commend)' class=\"sbt\">\n
&nbsp;&nbsp;<input type=\"submit\" name=\"Submit\" value='连放' class=\"sbt\"></div></li> ";
$pages = numofpage($songsum,$page,ceil($songsum/$perpage),'commend.php?');	
echo $pages;
echo"<!--\n";
}
