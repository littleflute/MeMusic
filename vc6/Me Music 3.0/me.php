...
require("global.php");
$randnum=isset($_GET['randnum']) ? $_GET['randnum'] :50;
$mmlist=@file("".$datadir."/list.php");
$count=count($mmlist);

$MaxSongId=$count-1;
srand ((float)microtime()*1000000); 
for($i=0;$i<$randnum;$i++){
	$randlist[]= rand(1,$MaxSongId); 
}
...
for($i=0;$i<$count;$i++)
   {  $flag=0 ;
   list($catid,$singerid,$songid,$songname,$addtime)=explode("|",$mmlist[$i]);
     for($j=0;$j<$randnum;$j++)
	  if($randlist[$j]==$songid) {
	    $flag=1; break ;
		}
	 if($flag==0) continue; 
